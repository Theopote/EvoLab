...
export const scoreDaylight = (context: ScoringContext): MetricResult => {
  ...
  daylightRooms.forEach((room) => {
    ...
    const roomScore = touchesExterior && hasOpening ? (depthOk ? 100 : Math.max(35, 100 - ((depth - maxDepth) / maxDepth) * 45)) : 0;
    ...
    evidence.push({
      label: room.name,
      value: `${touchesExterior && hasOpening ? "windowed" : "no window"}, depth ${round1(depth)}m`,
  ...
  return {
    ...
    hints:
      windowFailures > 0
        ? [`${windowFailures} daylight room(s) lack exterior windows.`]
        : depthFailures > 0
          ? [`${depthFailures} room(s) exceed recommended depth of ${maxDepth}m.`]
          : undefined
  };
};
