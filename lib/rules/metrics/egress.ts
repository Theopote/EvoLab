...
const isGraphRouted = (method: string) => method !== "centroid-fallback";
...
export const scoreEgress = (context: ScoringContext): MetricResult => {
  ...
  const pathRooms = metrics.perRoom.filter((item) => isGraphRouted(item.method)).length;
  const portalRouted = metrics.perRoom.filter((item) => item.method === "opening-aware-path").length;
  ...
  return {
    ...
    evidence: [
      ...
      { label: "Graph-routed rooms", value: `${pathRooms}/${metrics.perRoom.length || 0}` },
      ...(portalRouted > 0
        ? [{ label: "Portal-routed rooms", value: `${portalRouted}/${metrics.perRoom.length || 0}`, impact: "positive" as const }]
        : [])
    ],
    hints:
      metrics.maxDistance > maxDistance
        ? [`${metrics.worstRoomName ?? "A room"} exceeds egress limit by ${round1(overrun)}m.`]
        : metrics.method === "opening-aware-path"
          ? ["Egress distances follow door portal paths through circulation and vestibules."]
          : undefined
  };
};
