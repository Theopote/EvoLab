...
export function insetPolygon(polygon: Point[], distance: number, options: PolygonOffsetOptions = {}) {
  return offsetPolygon(polygon, -Math.abs(distance), options);
}

export function outsetPolygon(polygon: Point[], distance: number, options: PolygonOffsetOptions = {}) {
  return offsetPolygon(polygon, Math.abs(distance), options);
}
