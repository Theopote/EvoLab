import { polygonArea, unitePolygons } from "@/lib/geometry/kernel";
import { deriveWallGraph } from "@/lib/geometry/topology/wall-graph";
...
export function roomsShareInteriorWall(roomA: Room, roomB: Room) {
  ...
  return graph.edges.some((edge) => edge.roomIds.includes(roomA.id) && edge.roomIds.includes(roomB.id));
}
