...
interface ModifyPlanResponse {
  ...
  version: PlanVersion;           // 引擎应用全部 operations 后的预览版
...
- `mode: "proposal"`：优先路径；UI 应展示 `PlanChangeProposalPanel`，默认勾选全部 operation，用户可取消后 **仅应用已勾选项**。
- `mode: "legacy"`：LLM 仍返回完整版本时的兼容回退（逐步淘汰）。

客户端用 `applyPlanOperations(base, operations, acceptedIds)` 在本地重算最终版本，避免二次请求。

## 与现有 ChangeSet 的关系

1. Copilot 生成 `proposal` + `previewVersion`（draft，不立即覆盖用户正在编辑的版本）。
2. 用户确认后，客户端用 **已接受的 operations** 生成 `finalVersion`。
3. `updateActiveVersion(finalVersion, { source: "ai", summary: proposal.intent })` 触发现有 `recordVersionChangeSet`。
4. Quantity 面板的 `ChangeSetApprovalPanel` 继续负责整包 approve/reject；operation 级审批在 Copilot 侧完成。

长期：ChangeSet 可挂载 `proposalId` 与 `operationIds`，实现跨模块统一审批。
...
- 多人协作：proposal 持久化、评论、operation 级 audit log
- 淘汰 `legacy` 整版重写路径
...
4. **渐进增强** — legacy 回退保证现有演示与 mock 不断裂。
