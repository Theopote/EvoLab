...
export function RenderPanel({ activeVersion }: RenderPanelProps) {
  ...
  const [notes, setNotes] = useState("Professional architectural visualization, restrained BIM/CAD design language, readable massing and clear spatial hierarchy.");
    ...
    <section className="grid min-h-full grid-cols-[360px_minmax(0,1fr)] gap-4">
      <aside className="rounded border border-line bg-panel/90 p-3">
        <div className="mb-4 flex items-center justify-between">
          <div>
            <h1 className="text-base font-semibold text-white">Render Setup</h1>
            <p className="mt-1 text-xs text-muted">Render briefs stay linked to the editable model.</p>
          </div>
        ...
        <div className="mt-4 rounded border border-line bg-[#0b1118] p-3">
          <div className="mb-2 flex items-center justify-between">
            <h2 className="text-sm font-medium text-white">Generated brief</h2>
            <button
          ...
          <pre className="max-h-64 whitespace-pre-wrap rounded bg-black/20 p-2 text-xs leading-5 text-muted">
            {renderBrief}
          </pre>
        </div>
      </aside>
      ...
      <section className="grid min-h-full grid-rows-[auto_minmax(560px,1fr)] gap-4">
        <div className="rounded border border-line bg-panel/90 p-3">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-base font-semibold text-white">Render Preview Model</h2>
              <p className="mt-1 text-xs text-muted">
                The preview uses the same activeVersion geometry as the Model tab.
              </p>
            </div>
